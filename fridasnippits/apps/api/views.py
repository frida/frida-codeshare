...
def like_project(request):
    try:
        ...
        if request.user.liked_projects.filter(project_id=project_uuid).exists():
            return JsonResponse(
                {"success": False, "error": "Already liked this project!"}, status=400
            )
...
def update_project(request, nickname, project_slug):
    try:
        ...
        if project_owner != request.user:
            return JsonResponse(
                {"success": False, "error": "You're not allowed to edit this!"},
                status=400,
            )
    except:
        return JsonResponse(
            {"success": False, "error": "You're not allowed to edit this!"}, status=400
        )
    ...
    try:
        ...
        project.hash = hashlib.sha256(
            cleaned_data["source"].encode("utf-8")
        ).hexdigest()
