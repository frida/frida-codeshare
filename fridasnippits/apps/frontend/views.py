...
def index(request):
    ...
    return render(
        request,
        "index.html",
        {"projects": projects, "projects_are_odd": len(projects) % 2 == 1},
    )
...
def browse(request):
    ...
    return render(
        request,
        "browse.html",
        {"projects": results, "projects_are_odd": len(results) % 2 == 1},
    )
...
def new(request):
    return render(
        request,
        "new.html",
        {"categories": Category.objects.all(), "disable_sidebar": True},
    )
...
def project_view(request, nickname, project_slug):
    try:
        ...
        project = Project.objects.get(
            owner=project_user, project_slug=project_slug.lower()
        )
    ...
    return render(
        ...
        "project_view.html",
        {
            ...
            "fingerprint": hashlib.sha256(
                project.project_source.encode("utf-8")
            ).hexdigest(),
        },
    )
